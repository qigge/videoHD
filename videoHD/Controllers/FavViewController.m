...
- (void)createView {
    ...
    _favTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-64) style:UITableViewStylePlain];
...
- (void)getDataFromDb {
    ...
    [manager.database inDatabase:^(FMDatabase *db) {
        FMResultSet *resultSet = [db executeQuery:@"select * from fav left join course on fav.plid = course.plid"];
...
- (void)editBtn:(UIButton *)btn {
    ...
    if (_isEdit) {
        // 是编辑
        _favTableView.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-50);
        [btn setTitle:@"取消" forState:UIControlStateNormal];
    }else {
        _favTableView.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height);
...
- (void)delete:(UIButton *)btn {
    ...
    [manager.database inDatabase:^(FMDatabase *db) {
        for (NewCourseModel *model in _editArr) {
            if ([db executeUpdate:[NSString stringWithFormat:@"delete from fav where plid = '%@'",model.plid]]) {
                NSLog(@"delete");
            }
        }
        [_editArr removeAllObjects];
